...
class Ui_MainWindow(object):
    ...
    def process(self, surnameInput, locationInput, csvInput):
        try:
            if surnameInput.strip() == "":
                raise RequireValueException("name Required")
            elif locationInput.strip() == "":
                raise RequireValueException("location Required")
            elif csvInput.strip() == Ui_MainWindow.currentDirectory + "/contacts/":
                raise RequireValueException("Please specify output file name")

            path = "main.exe --name {} --location {} --output {}".format(surnameInput, locationInput, csvInput)
    ...
    def processBtnAction(self):
        surnameInput = self.surnameInput.text()
        locationInput = self.locationInput.text()
        # streetInput = self.streetInput.text()
        csvInput = Ui_MainWindow.currentDirectory + "/contacts/" + self.outputInput.text()
        self.progress()
        ## starting thread
        thread.start_new_thread(self.process, (surnameInput, locationInput, csvInput, ))
    ...
    def setupUi(self, MainWindow):
        ...
        self.streetInput.setFocusPolicy(QtCore.Qt.ClickFocus)
        self.streetInput.setObjectName("streetInput")
        self.gridLayout_2.addWidget(self.streetInput, 4, 0, 1, 1)
        self.locationInput = QtWidgets.QLineEdit(self.frame)
        self.locationInput.setFocusPolicy(QtCore.Qt.ClickFocus)
        self.locationInput.setObjectName("locationInput")
        self.gridLayout_2.addWidget(self.locationInput, 2, 1, 1, 1)
        self.label_2 = QtWidgets.QLabel(self.frame)
        self.label_2.setObjectName("label_2")
        self.gridLayout_2.addWidget(self.label_2, 1, 0, 1, 1)
        self.surnameInput = QtWidgets.QLineEdit(self.frame)
        self.surnameInput.setFocusPolicy(QtCore.Qt.StrongFocus)
        self.surnameInput.setClearButtonEnabled(False)
        self.surnameInput.setObjectName("surnameInput")
        self.gridLayout_2.addWidget(self.surnameInput, 2, 0, 1, 1)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.streetInput.setPlaceholderText(_translate("MainWindow", "Enter Street to be specific (Optional)"))
        self.locationInput.setPlaceholderText(_translate("MainWindow", "Enter location to find contact from"))
        self.label_2.setText(_translate("MainWindow", "Surname"))
        self.surnameInput.setPlaceholderText(_translate("MainWindow", "Enter Surname to find names with"))
